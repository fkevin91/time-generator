...
let top_bar = ref(" ")
let type = ref("Devis n°")
let middle_bar = ref(" ")
let title_column_description = ref("Description")
let title_column_qte = ref("Qté")
let title_column_prix_unitaire = ref("Prix unitaire")
let title_column_total = ref("Total H.T")
...
async function previewDevis() {
    ...
    inputs.forEach(elt => {
      ...
      elt.title_column_prix_unitaire = title_column_prix_unitaire.value,
      elt.title_column_total = title_column_total.value
    });
...
<template>
  <div class="space-y-3">
    ...
    <div v-if="!formulaire" class="flex justify-between">
      ...
      <button @click="updateDevis" class="text-white p-3 bg-yellow-500 rounded-lg">Enregistrer les modifications</button>
